...
RELEASE_ALLOWLIST = [path for group in RELEASE_ALLOWLIST_GROUPS.values() for path in group]
FORBIDDEN_PATTERNS = [
    ".git/",
    "release/",
    "release_ci*/",
...
def build_zip(output_dir: Path, zip_name: str | None = None) -> Path:
    ...
    for relative in RELEASE_ALLOWLIST:
        ...
        if source.is_dir():
            raise IsADirectoryError(f"Release allowlist must not include directories: {relative}")
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for relative in RELEASE_ALLOWLIST:
            archive.write(staging_dir / relative, arcname=relative.replace(os.sep, "/"))
    ...
    for name in names:
        ...
        for pattern in FORBIDDEN_PATTERNS:
            ...
            if "/" not in stripped and "*" not in stripped and stripped in parts:
                hits.append(normalized)
                break
            if stripped.endswith("/") and any(part == stripped.rstrip("/") for part in parts):
                hits.append(normalized)
                break
    return hits
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create the official Smart Organizer runtime/demo release zip.")
    parser.add_argument("--output-dir", default="release")
    parser.add_argument("--zip-name", default="")
    args = parser.parse_args()

    zip_path = build_zip(PROJECT_ROOT / args.output_dir if not Path(args.output_dir).is_absolute() else Path(args.output_dir), args.zip_name or None)
