...
- 建議：Python `3.11`
- 支援並驗證：Python `3.11`、`3.12`
- 不支援的版本會在啟動與 release validation preflight 提早失敗。Python `3.10` 與 `3.13+` 不支援。
...
如果瀏覽器或 Streamlit session 在上傳分析後、正式整理前關閉，未完成的上傳可在 **Search & Records > Maintenance > Unfinished Uploads** 復原。使用 **Resume** 還原已保存的檢視資料，使用 **Re-analyze** 從保存的暫存檔重新分析，或使用 **Discard** 移除未完成資料庫紀錄與核准的暫存檔。若暫存檔已遺失，紀錄會標記為 broken，仍可安全捨棄。
...
Smart Organizer 預設會把可變動的 runtime 資料放在 source repository 外。若要指定其他可寫入位置，請設定 `SMART_ORGANIZER_DATA_DIR`。
